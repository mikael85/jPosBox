...
public class PosPrinter {
    ...
    void HeaderOfLines() {
        lineaguion();
        if (html == false) {
            P = P + ResizeText("Description", 27) + " " + ResizeText("Qty", 6) + " " + ResizeText("Total", 6);
        } else {
            P = P + "<table cellspacing=\"0px\"><tr><td>Description</td><td>Qty</td><td>Total</td></tr>";
        }
        salto();
        if (html == false) {
            lineaguion();
    ...
    void HeaderOfLinesNoTotal() {
        lineaguion();
        if (html == false) {
            P = P + ResizeText("Description", 27) + " " + ResizeText("Qty", 6);
        } else {
            P = P + "<table cellspacing=\"0px\"><tr><td>Description</td><td>Qty</td><td></td></tr>";
        }
        salto();
        if (html == false) {
            lineaguion();
    ...
    void addline(String label, String qty, String total) {
        ...
        if (html == false) {
            P = P + ResizeText(label, 27) + " " + ResizeText(qty, 6) + " " + ResizeText(total, 6);
        } else {
            P = P + "<tr><td>" + label + "</td><td align=\"right\">" + qty + "</td><td align=\"right\">" + total + "</td></tr>";
        }
        salto();
    }
    ...
    void print(String printer, int copies, String FontSize) {
        if (printer.equals("nada")) {
            out.println("Printer not configured");
            return;
        }
        ...
        if (html == false) {
            ...
            try {
                ...
                ed.setEditorKit(kit);
                ed.setDocument(doc);
                ed.setText("<html><head><style>body{font-size:" + FontSize + "px;}</style></head><body>" + P + "</body></html>");
    ...
    void print_raw(String printer, String FontSize, String raw_html) {
        if (printer.equals("nada")) {
            out.println("Printer not configured");
            return;
        }
